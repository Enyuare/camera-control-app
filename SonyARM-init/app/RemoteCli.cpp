...
void processJsonCommand(const std::string& jsonStr) {
    Json::Reader reader;
    Json::Value obj;
...
int main()
{
    ...
    std::thread serverThread(startSocketServer);
            ...
            while (true)
            {
                ...
                // loop-Menu6
                    ...
                    while (true) {
                        ...
                        commandCondition.wait(lock, []{ return !receivedCommand.empty(); });

                        //performAction(receivedCommand);
                        select = receivedCommand;
